...
export default function ImageInverterToolPt() {
  ...
  // 多语言支持 / Multilingual support - 固定为葡萄牙语
  const t: Translations = getTranslations('pt');
  ...
  const handleDownload = useCallback(() => {
    if (processedImage) {
      downloadImage(processedImage, 'imagem-invertida.png');
    }
  }, [processedImage]);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <label className="flex items-center">
                        ...
                        />
                        <span className="text-sm text-gray-600 dark:text-gray-400">Ativar preservação de transparência</span>
                      </label>
                    </div>
                      ...
                      {processingResult && (
                        <div className="mt-4 p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-100 dark:border-purple-800">
                          ...
                          <div className="grid grid-cols-2 gap-3 text-xs text-purple-600 dark:text-purple-400">
                            <div><strong>Tamanho:</strong> {processingResult.originalSize.width} × {processingResult.originalSize.height}px</div>
                            <div><strong>Tempo:</strong> {processingResult.processingTime.toFixed(2)}ms</div>
                            <div><strong>Modo:</strong> {t.presets[selectedPreset as keyof typeof t.presets] || selectedPreset}</div>
                            <div><strong>Intensidade:</strong> {Math.round((customOptions.intensity || 1) * 100)}%</div>
                          </div>
                        </div>
                      )}
                    </>

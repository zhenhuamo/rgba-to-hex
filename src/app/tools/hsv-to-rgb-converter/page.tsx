...
import { RGB, hsvToRgb, hsvToHex } from '@/utils/colorConverter';

// 添加全局样式，确保深色/浅色模式切换时有平滑过渡
const GlobalStyles = () => {
  return (
    <style jsx global>{`
      * {
...
const HsvToRgbConverterTool = () => {
  ...
  const [hex, setHex] = useState<string>('#FF0000'); // 添加HEX状态
  const [isCopied, setIsCopied] = useState<boolean>(false); // State for copy feedback
  const [copyType, setCopyType] = useState<string>(''); // 跟踪复制的内容类型
    ...
    <div data-embed="true" className="p-4 bg-transparent">
     ...
     <div className="p-4 sm:p-6 max-w-2xl mx-auto transition-all duration-500">
       ...
       <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-5 sm:p-6 mb-6 transition-all duration-500 dark:border dark:border-gray-700">
         <h1 className="text-2xl font-bold text-center text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600 mb-6">HSV to RGB Converter</h1>
         {ConverterUI}
       </div>

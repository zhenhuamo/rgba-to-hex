...
import { filterBlackColors } from '../utils/blackColorUtils';

interface UseBlackColorDataReturn {
  colors: ColorData[];
...
const CACHE_KEY = 'black-colors-processed';
const CACHE_DURATION = 24 * 60 * 60 * 1000; // 24小时
const ITEMS_PER_PAGE = 50; // 每页显示的颜色数量

export function useBlackColorData(): UseBlackColorDataReturn {
    ...
    try {
      ...
      if (cached) {
        try {
          ...
          console.warn('Failed to parse cached black colors:', e);
      ...
      // 处理黑色数据
      const processedColors = await processBlackColorsInBatches(rawColors);
      ...
      try {
        ...
        console.warn('Failed to cache black colors:', e);
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load black colors';
      setError(errorMessage);
      console.error('Error loading black colors:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 分批处理颜色数据以避免阻塞UI
  const processBlackColorsInBatches = async (
    rawColors: { name: string; hex: string }[]
  ): Promise<ColorData[]> => {
    const batchSize = 100;
    const allBlackColors: ColorData[] = [];

    for (let i = 0; i < rawColors.length; i += batchSize) {
      const batch = rawColors.slice(i, i + batchSize);
      const batchBlackColors = filterBlackColors(batch);
      allBlackColors.push(...batchBlackColors);
    ...
    // 按流行度和名称排序
    return allBlackColors.sort((a, b) => {

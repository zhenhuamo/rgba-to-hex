...
import { filterWhiteColors } from '../utils/whiteColorUtils';

interface UseWhiteColorDataReturn {
  colors: ColorData[];
...
const CACHE_KEY = 'white-colors-processed';
const CACHE_DURATION = 24 * 60 * 60 * 1000; // 24小时
const ITEMS_PER_PAGE = 50; // 每页显示的颜色数量

export function useWhiteColorData(): UseWhiteColorDataReturn {
    ...
    try {
      ...
      if (cached) {
        try {
          ...
          console.warn('Failed to parse cached white colors:', e);
      ...
      // 处理白色数据
      const processedColors = await processWhiteColorsInBatches(rawColors);
      ...
      try {
        ...
        console.warn('Failed to cache white colors:', e);
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load white colors';
      setError(errorMessage);
      console.error('Error loading white colors:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 分批处理颜色数据以避免阻塞UI
  const processWhiteColorsInBatches = async (
    rawColors: { name: string; hex: string }[]
  ): Promise<ColorData[]> => {
    const batchSize = 100;
    const allWhiteColors: ColorData[] = [];

    for (let i = 0; i < rawColors.length; i += batchSize) {
      const batch = rawColors.slice(i, i + batchSize);
      const batchWhiteColors = filterWhiteColors(batch);
      allWhiteColors.push(...batchWhiteColors);
    ...
    // 按流行度和名称排序
    return allWhiteColors.sort((a, b) => {

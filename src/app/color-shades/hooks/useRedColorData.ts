...
import { filterRedColors } from '../utils/redColorUtils';

interface UseRedColorDataReturn {
  colors: ColorData[];
...
const CACHE_KEY = 'red-colors-processed';
const CACHE_DURATION = 24 * 60 * 60 * 1000; // 24小时
const ITEMS_PER_PAGE = 50; // 每页显示的颜色数量

export function useRedColorData(): UseRedColorDataReturn {
    ...
    try {
      ...
      if (cached) {
        try {
          ...
          console.warn('Failed to parse cached red colors:', e);
      ...
      // 处理红色数据
      const processedColors = await processRedColorsInBatches(rawColors);
      ...
      try {
        ...
        console.warn('Failed to cache red colors:', e);
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load red colors';
      setError(errorMessage);
      console.error('Error loading red colors:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 分批处理颜色数据以避免阻塞UI
  const processRedColorsInBatches = async (
    rawColors: { name: string; hex: string }[]
  ): Promise<ColorData[]> => {
    const batchSize = 100;
    const allRedColors: ColorData[] = [];

    for (let i = 0; i < rawColors.length; i += batchSize) {
      const batch = rawColors.slice(i, i + batchSize);
      const batchRedColors = filterRedColors(batch);
      allRedColors.push(...batchRedColors);

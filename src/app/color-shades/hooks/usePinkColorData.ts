...
import { filterPinkColors } from '../utils/pinkColorUtils';

interface UsePinkColorDataReturn {
  colors: ColorData[];
...
const CACHE_KEY = 'pink-colors-processed';
const CACHE_DURATION = 24 * 60 * 60 * 1000; // 24小时
const ITEMS_PER_PAGE = 50; // 每页显示的颜色数量

export function usePinkColorData(): UsePinkColorDataReturn {
    ...
    try {
      ...
      if (cached) {
        try {
          ...
          console.warn('Failed to parse cached pink colors:', e);
      ...
      // 处理粉色数据
      const processedColors = await processPinkColorsInBatches(rawColors);
      ...
      try {
        ...
        console.warn('Failed to cache pink colors:', e);
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load pink colors';
      setError(errorMessage);
      console.error('Error loading pink colors:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 分批处理颜色数据以避免阻塞UI
  const processPinkColorsInBatches = async (
    rawColors: { name: string; hex: string }[]
  ): Promise<ColorData[]> => {
    const batchSize = 100;
    const allPinkColors: ColorData[] = [];

    for (let i = 0; i < rawColors.length; i += batchSize) {
      const batch = rawColors.slice(i, i + batchSize);
      const batchPinkColors = filterPinkColors(batch);
      allPinkColors.push(...batchPinkColors);

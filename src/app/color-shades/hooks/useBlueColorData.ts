...
import { filterBlueColors } from '../utils/blueColorUtils';

interface UseBlueColorDataReturn {
  colors: ColorData[];
...
const CACHE_KEY = 'blue-colors-processed';
const CACHE_DURATION = 24 * 60 * 60 * 1000; // 24小时
const ITEMS_PER_PAGE = 50; // 每页显示的颜色数量

export function useBlueColorData(): UseBlueColorDataReturn {
    ...
    try {
      ...
      if (cached) {
        try {
          ...
          console.warn('Failed to parse cached blue colors:', e);
      ...
      // 处理蓝色数据
      const processedColors = await processBlueColorsInBatches(rawColors);
      ...
      try {
        ...
        console.warn('Failed to cache blue colors:', e);
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load blue colors';
      setError(errorMessage);
      console.error('Error loading blue colors:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 分批处理颜色数据以避免阻塞UI
  const processBlueColorsInBatches = async (
    rawColors: { name: string; hex: string }[]
  ): Promise<ColorData[]> => {
    const batchSize = 100;
    const allBlueColors: ColorData[] = [];

    for (let i = 0; i < rawColors.length; i += batchSize) {
      const batch = rawColors.slice(i, i + batchSize);
      const batchBlueColors = filterBlueColors(batch);
      allBlueColors.push(...batchBlueColors);

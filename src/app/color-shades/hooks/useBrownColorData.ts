...
import { filterBrownColors } from '../utils/brownColorUtils';

interface UseBrownColorDataReturn {
  colors: ColorData[];
...
const CACHE_KEY = 'brown-colors-processed';
const CACHE_DURATION = 24 * 60 * 60 * 1000; // 24小时
const ITEMS_PER_PAGE = 50; // 每页显示的颜色数量

export function useBrownColorData(): UseBrownColorDataReturn {
    ...
    try {
      ...
      if (cached) {
        try {
          ...
          console.warn('Failed to parse cached brown colors:', e);
      ...
      // 处理棕色数据
      const processedColors = await processBrownColorsInBatches(rawColors);
      ...
      try {
        ...
        console.warn('Failed to cache brown colors:', e);
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load brown colors';
      setError(errorMessage);
      console.error('Error loading brown colors:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 分批处理颜色数据以避免阻塞UI
  const processBrownColorsInBatches = async (
    rawColors: { name: string; hex: string }[]
  ): Promise<ColorData[]> => {
    const batchSize = 100;
    const allBrownColors: ColorData[] = [];

    for (let i = 0; i < rawColors.length; i += batchSize) {
      const batch = rawColors.slice(i, i + batchSize);
      const batchBrownColors = filterBrownColors(batch);
      allBrownColors.push(...batchBrownColors);
    ...
    // 按流行度和名称排序
    return allBrownColors.sort((a, b) => {

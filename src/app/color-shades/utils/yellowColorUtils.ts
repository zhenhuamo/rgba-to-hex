...
import { YellowShadeCategory } from '../types/yellowShades';
import { YELLOW_SHADE_CATEGORIES } from '../constants/yellowShadeCategories';

// 判断颜色是否为黄色系
export function isYellowColor(hex: string): boolean {
  const color = colord(hex);
  const hsl = color.toHsl();
  
  // 黄色色相范围：45-75度，饱和度至少15%
  return hsl.h >= 45 && hsl.h <= 75 && hsl.s >= 15;
}

// 从原始颜色数据中筛选黄色系
export function filterYellowColors(colors: { name: string; hex: string }[]): ColorData[] {
  return colors
    .filter(color => isYellowColor(color.hex))
...
export function processColorData(rawColor: { name: string; hex: string }): ColorData | null {
  try {
    ...
    // 确保是黄色系
    if (!isYellowColor(rawColor.hex)) {
      return null;
    }
    
    const category = categorizeYellowShade(hsl);
    if (!category) {
      return null;
    }
...
// 黄色分类
export function categorizeYellowShade(hsl: HSLColor): YellowShadeCategory | null {
  for (const category of YELLOW_SHADE_CATEGORIES) {
  ...
  // 默认分类为中等黄色
  return YELLOW_SHADE_CATEGORIES.find(cat => cat.id === 'medium-yellow') || null;
}
...
export function generateColorTags(name: string, hsl: HSLColor): string[] {
  ...
  // 黄色特定标签
  tags.push('yellow');
  if (hsl.h >= 45 && hsl.h <= 55) tags.push('golden', 'gold');
  if (hsl.h >= 55 && hsl.h <= 65) tags.push('lemon', 'citrus');
  if (hsl.h >= 65 && hsl.h <= 75) tags.push('lime', 'green');
  if (hsl.l > 85 && hsl.s < 50) tags.push('cream', 'vanilla');
...
export function calculatePopularity(name: string): number {
  const popularKeywords = [
    'yellow', 'gold', 'golden', 'lemon', 'cream', 'vanilla', 'butter',
    'canary', 'sunshine', 'amber', 'mustard', 'honey', 'corn', 'banana'
  ];
...
// 查找相似黄色
export function findSimilarYellowColors(
  targetColor: string,
  colors: ColorData[],
  count: number = 6
): ColorData[] {
